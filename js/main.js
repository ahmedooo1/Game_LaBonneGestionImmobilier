...
function setupEventListeners() {
    ...
    if (historyButton) {
        ...
        console.error('Le bouton avec ID "show-history-button" n\'est pas trouvé dans le HTML !');
    }
...
function nextTurn() {
    ...
    if (currentPlayer) {
        updatePlayerTurn(currentPlayer, `Fin du tour: Passage au joueur suivant`);
    }
...
function handleBonusCard() {
    ...
    document.getElementById('bonus-continue').onclick = () => {
        ...
        if (bonusCard.type === 'special') {
            ...
            const currentPlayer = activeTeam.players[activeTeam.currentPlayer];
            if (currentPlayer) {
                updatePlayerTurn(currentPlayer, `Tour: +${bonusCard.amount} K pour bonus ${bonusCard.title}`);
            }
            // Log after update to verify
...
function handleBiensCard() {
    ...
    const currentPlayer = activeTeam.players[activeTeam.currentPlayer];
    if (currentPlayer) {
        updatePlayerTurn(currentPlayer, `Tour: -${amount} K pour Bien, ${biensCard.title}`);
    }
...
function handleInteractionCard() {
    ...
    const currentPlayer = activeTeam.players[activeTeam.currentPlayer];
    if (currentPlayer) {
        updatePlayerTurn(currentPlayer, `Tour: -${amount} K pour Interaction ${interactionCard.title}`);
    }
                            // Log after update to verify
...
function handlePDBCard() {
    ...
    const currentPlayer = activeTeam.players[activeTeam.currentPlayer];
    if (currentPlayer) {
        updatePlayerTurn(currentPlayer, `Tour: -${amount} K pour PDB ${pdbCard.title}`);
    }
